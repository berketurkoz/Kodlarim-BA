...
namespace Donguler_Rezervasyon
{
    public partial class Form1 : Form
    {
        ...
        private void lbKoltuTipi_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lbKoltuTipi.SelectedItem == "Hiçbiri")
            {
                foreach (Control control in scAnaPanel.Panel2.Controls)
                {
                    if (control is Button)
            ...
            else if (lbKoltuTipi.SelectedItem == "Erkek")
            {
                foreach (Control control in scAnaPanel.Panel2.Controls)
                {
                    if (control is Button)
                    {
                        Button btn = (control as Button);
                        Koltuk koltuk = ((Koltuk)btn.Tag);
                        // Yan koltuğu erkek olanların Enable olan yapacağız.
                        Koltuk yanKoltuk = YanKoltuguGetir(koltuk);
                        if (yanKoltuk.cinsiyetTipi == CinsiyetTipi.Erkek)
                            btn.Enabled = true;
                        else
            ...
            else if (lbKoltuTipi.SelectedItem == "Kadın")
            {
                foreach (Control control in scAnaPanel.Panel2.Controls)
                {
                    if (control is Button)
